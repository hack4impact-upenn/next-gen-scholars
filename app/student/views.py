...
def edit_test_score(item_id):
    test_score = TestScore.query.filter_by(id=item_id).first()
    if test_score:
        form = EditTestScoreForm(test_name=test_score.name, month=test_score.month, 
            year=test_score.year, score=test_score.score)
        if form.validate_on_submit():
...
def add_recommendation_letter():
    ...
    return render_template('student/add_academic_info.html', form=form, 
...
def edit_recommendation_letter(item_id):
    ...
    if letter:
        form = EditRecommendationLetterForm(name=letter.name, category=letter.category,
...
def update_checklist_item(item_id):
    ...
    if item:
        ...
        return render_template('student/update_checklist.html', form=form,
            student_profile_id=item.assignee_id)
    flash('Item could not be updated', 'error')
    return redirect(url_for('main.index'))
    sat = '––'
    ...
    if student_profile is not None:
        ...
        return render_template('student/student_profile.html',
            user=current_user, sat=sat, act=act)
